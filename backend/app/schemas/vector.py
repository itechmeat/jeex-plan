...
class VectorPayload(BaseModel):
    ...
    type: DocumentType = Field(
        default=DocumentType.KNOWLEDGE, description="Type of stored information"
    )
    visibility: VisibilityLevel = Field(
        default=VisibilityLevel.PRIVATE, description="Access visibility level"
    )
    ...
    content_type: str | None = Field(
        None, description="MIME type or content classification"
    )
    ...
    confidence_score: float | None = Field(
        None, ge=0.0, le=1.0, description="Content confidence score"
    )
...
class SearchResult(BaseModel):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {"id": self.id, "score": self.score, "payload": self.payload.to_dict()}
...
class SearchRequest(BaseModel):
    ...
    # Optional filters
    doc_types: list[DocumentType] | None = Field(
...
class UpsertRequest(BaseModel):
    ...
    payloads: list[dict[str, Any]] = Field(
        ..., description="List of payload dictionaries"
    )
    doc_type: DocumentType = Field(
        default=DocumentType.KNOWLEDGE, description="Document type"
    )
    visibility: VisibilityLevel = Field(
        default=VisibilityLevel.PRIVATE, description="Visibility level"
    )
    version: str = Field(default="1.0", description="Document version")
...
class DeleteRequest(BaseModel):
    ...
    point_ids: list[str] | None = Field(
        None, description="Specific point IDs to delete"
    )
    doc_types: list[DocumentType] | None = Field(
        None, description="Filter by document types"
    )
...
class CollectionStats(BaseModel):
    ...
    tenant_project_points: int | None = Field(
        None, description="Points for specific tenant/project"
    )
