...
class ProjectContext(BaseModel):
    ...
    current_step: int = Field(
        ..., ge=1, le=4, description="Current workflow step (1-4)"
    )
    correlation_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Request correlation ID"
    )
    language: str = Field(
        default="en", description="Target language for generated documents"
    )
...
class AgentOutput(BaseModel, ABC):
    ...
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Agent confidence in output quality"
    )
