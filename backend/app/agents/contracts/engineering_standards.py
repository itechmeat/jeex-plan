...
class CodeStandard(BaseModel):
    ...
    category: str = Field(..., description="Standard category (naming, structure, etc.)")
    rules: List[str] = Field(..., description="Specific rules and guidelines")
    examples: Optional[Dict[str, str]] = Field(None, description="Code examples (good/bad)")
    tools: List[str] = Field(default_factory=list, description="Recommended tools for enforcement")
...
class EngineeringStandardsInput(AgentInput):
    ...
    existing_standards: Optional[Dict[str, Any]] = Field(None, description="Existing team standards")
...
class EngineeringStandardsOutput(AgentOutput):
    ...
    coding_standards: List[CodeStandard] = Field(..., description="Code quality standards")
    review_process: str = Field(..., description="Code review process and guidelines")
    definition_of_done: List[str] = Field(..., description="Definition of Done criteria")
    ...
    performance_standards: List[str] = Field(..., description="Performance requirements")
    documentation_requirements: List[str] = Field(..., description="Documentation standards")
