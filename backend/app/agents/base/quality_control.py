...
class MarkdownValidator(ContentValidator):
    ...
    async def validate(
        ...
        if not has_h3:
            suggestions.append(
                "Consider adding H3 headers (###) for detailed structure"
            )
        ...
        overall_score = min(
            1.0,
            (structure_score + min(1.0, content_score) + quality_score) / 3
            + formatting_bonus,
        )
    ...
    async def validate(
        ...
        try:
            ...
            if flesch_ease < self.min_reading_ease:
                suggestions.append(
                    "Content may be too complex - consider shorter sentences"
                )
            ...
            if avg_sentence_length > 25:
                suggestions.append(
                    "Average sentence length is high - break up long sentences"
                )
            ...
            length_score = min(
                1.0, word_count / 1000
            )  # Normalize by expected word count

            # Calculate syllable complexity (lower is better for readability)
            avg_syllables_per_word = (
                syllable_count / word_count if word_count > 0 else 0
            )
            syllable_score = max(
                0.0, 1.0 - (avg_syllables_per_word - 1.5) / 2
            )  # Optimal around 1.5 syllables per word

            overall_score = (
                ease_score + grade_score + length_score + syllable_score
            ) / 4
...
class BusinessAnalystValidator(ContentValidator):
    ...
    async def validate(
        ...
        has_metrics = any(
            ...
            for keyword in ["kpi", "metric", "measure", "target"]
        )
        has_audience = any(
            keyword in content_lower
            for keyword in ["user", "customer", "audience", "stakeholder"]
        )
        has_value_prop = any(
            keyword in content_lower
            for keyword in ["value", "benefit", "advantage", "solution"]
        )
...
class SolutionArchitectValidator(ContentValidator):
    ...
    async def validate(
        ...
        has_technologies = any(
            tech in content_lower
            for tech in ["database", "frontend", "backend", "api", "server", "client"]
        )
        has_patterns = any(
        ...
        try:
            ...
            for validator_name in validators_to_run:
                if validator_name in self.validators:
                    result = await self.validators[validator_name].validate(
                        output.content
                    )
