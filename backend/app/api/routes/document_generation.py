...
class ExportRequest(BaseModel):
    ...
    expires_in_hours: int = Field(default=24, ge=1, le=168, description="Export expiration in hours")
...
async def _generate_export_background(export_service: ExportService, export_id: UUID) -> None:
    ...
    try:
        ...
        logger.info(f"Export {export_id} generated successfully")

    except Exception as e:
        logger.error(f"Failed to generate export {export_id}: {e}")
