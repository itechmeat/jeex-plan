...
class UserRepository(TenantRepository[User]):
    ...
    async def link_oauth_account(
        self, user_id: UUID, oauth_provider: str, oauth_id: str
    ) -> User | None:
        """Link OAuth account to existing user."""
        return await self.update(
            user_id, oauth_provider=oauth_provider, oauth_id=oauth_id
        )

    async def unlink_oauth_account(self, user_id: UUID) -> User | None:
        """Unlink OAuth account from user."""
        return await self.update(user_id, oauth_provider=None, oauth_id=None)

    async def check_oauth_availability(
        self, oauth_provider: str, oauth_id: str, exclude_user_id: UUID | None = None
    ) -> bool:
    ...
    async def get_users_by_oauth_provider(
        self, oauth_provider: str, skip: int = 0, limit: int = 100
    ) -> list[User]:
        """Get all users with specific OAuth provider within tenant."""
        return await self.get_all(
            skip=skip, limit=limit, filters={"oauth_provider": oauth_provider}
        )
    ...
    async def update_last_login(self, user_id: UUID) -> User | None:
        ...
        return await self.update(user_id, updated_at=datetime.now(UTC))
