...
> **Управление секретами.** Рабочие токены Vault задаются только через локальные переменные окружения (`VAULT_TOKEN`, `VAULT_DEV_ROOT_TOKEN_ID`) или `.env.local`, которые исключены из VCS. Шаблон `backend/.env` содержит placeholder-значения и служит лишь подсказкой по структуре параметров.
